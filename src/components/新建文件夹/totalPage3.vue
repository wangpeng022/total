...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        this.$axios.post(publicu+"unifier/FNCenterBuildingListService", qs.stringify({"jsonString": '{}'})).then((res) => {
          if (res.data && res.data.content) {
      ...
      getList(param) {
        return this.$axios.post(publicu+"unifier/FNCenterGridService", qs.stringify({"jsonString": JSON.stringify(param)}))
      },
      downFile(param) {
        this.$axios({
          method: 'post',
